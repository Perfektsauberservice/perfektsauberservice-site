...
export default async (request, context) => {
  ...
  if (!city || !service) {
    return jsonResponse({
      ...
      acceptedCityExamples: cfg.cities.slice(0,4).map(x => x.slug),
      acceptedServiceExamples: cfg.services.slice(0,4).map(x => x.slug)
    }, 400);
  }
  return jsonResponse(buildPreviewPayload(city, service, cfg));
};
