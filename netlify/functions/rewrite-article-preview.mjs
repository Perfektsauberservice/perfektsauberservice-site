...
function listExistingBlogSlugs(cfg) {
  return uniq((cfg.pubState.items || []).map(x => x.slug));
}
...
export default async (request, context) => {
  ...
  const slug = body.slug || listExistingBlogSlugs(cfg)[0] || null;
  if (!slug) return jsonResponse({ ok:false, error:"No existing blog article found to rewrite." }, 404);
  return jsonResponse({
    ok: true,
    mode: "preview",
    slug,
    rewritePlan: {
      keepSlug: true,
      improve: ["meta title","meta description","intro","FAQ","CTA","GEO direct answers"],
      preserve: ["primary topic","city relevance","existing indexed URL"],
      imageSuggestionMode: "proposal only",
      updateScope: cfg.rules.updateScopes
    }
  });
};
