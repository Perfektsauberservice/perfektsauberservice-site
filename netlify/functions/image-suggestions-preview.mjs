...
export default async (request, context) => {
  ...
  if (!city || !service) return jsonResponse({ ok:false, error:"Unknown city or service" }, 400);
  return jsonResponse({
    ok: true,
    mode: "preview",
    imageApprovalRequired: cfg.rules.imageApprovalRequired,
    suggestions: imagePoolFor(city, service).slice(0, 5)
  });
};
