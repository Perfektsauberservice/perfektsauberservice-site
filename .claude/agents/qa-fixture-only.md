...
description: FIXTURE-ONLY-TEST variant of `qa` with zero tools granted at the frontmatter level (real technical enforcement, not a prompt request). Byte-identical to qa.md except this line, the tools: line below, and this description. Use this subagent_type — never `qa` — for any Acceptance/Extended suite run in fixture-only-test mode. Mirror integrity is checked by check-fixture-tooluse.mjs; edit qa.md, never this file, then re-copy.
model: sonnet
tools:
color: green
---
